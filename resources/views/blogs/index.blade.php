@extends('master')

@section('content')



<div class="ttm-page-title-row" style="background-image: url(../images/tassis14.jpg)!important;">
      <div class="container">
          <div class="row">
              <div class="col-md-12">
                  <div class="title-box text-center">
                      <div class="page-title-heading">
                          <h1 class="title text-light">لیست مقالات تسیس</h1>
                      </div><!-- /.page-title-captions -->
                      <div class="breadcrumb-wrapper">
                        <span class="text-light">مقالات</span>
                        <span class="ttm-bread-sep text-light">&nbsp; / &nbsp;</span>
                              <a class="text-light" title="Homepage" href="/">&nbsp;&nbsp;<i class="ti ti-home ml-1"></i>صفحه اصلی</a>


                      </span>

                      </div>
                  </div>
              </div><!-- /.col-md-12 -->
          </div><!-- /.row -->
      </div><!-- /.container -->
  </div><!-- page-title end-->

  <!--site-main start-->
  <div class="site-main">

      <!-- blog-section end -->
      <section class="ttm-row grid-section ttm-bgcolor-grey clearfix tasis-rtl">
          <div class="container">
              <!-- row -->
              <div class="row">
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
                  <div class="col-lg-4 col-md-6">
                      <!-- featured-imagebox-blog -->
                      <div class="featured-imagebox featured-imagebox-blog style2 mb-30">
                          <div class="featured-thumbnail"><!-- featured-thumbnail -->
                              <img class="img-fluid" src="images/blog/mk.jpg" alt="">
                              <div class="ttm-blog-overlay-iconbox">
                                  <a href="/blogs.show"><i class="ti ti-plus"></i></a>
                              </div>
                              <div class="ttm-box-view-overlay"></div>
                          </div>
                          <div class="featured-content"><!-- featured-content -->
                              <div class="ttm-box-post-date"><!-- ttm-box-post-date -->
                                  <span class="ttm-entry-date">
                                      <time class="entry-date tasis-byekan" datetime="2019-01-16T07:07:55+00:00">17<span class="entry-month entry-year">آبان</span></time>
                                  </span>
                              </div>
                              <div class="featured-title"><!-- featured-title -->
                                  <h5><a href="/blogs.show">موتورخانه</a></h5>
                              </div>
                              <div class="post-meta"><!-- post-meta -->

                                  <span class="ttm-meta-line"><i class="fa fa-user ml-1"></i>توسط: شیرازی</span>
                              </div>
                          </div>
                      </div><!-- featured-imagebox-blog end -->
                  </div>
              </div><!-- row end -->
              <div class="row">
                  <div class="col-md-12 text-center">
                      <div class="ttm-pagination">
                          <span class="page-numbers current tasis-byekan">1</span>
                          <a class="page-numbers tasis-byekan" href="#">2</a>
                          <a class="next page-numbers" href="#"><i class="ti ti-arrow-left"></i></a>
                      </div>
                  </div>
              </div>
          </div>
      </section>

        @endsection
